...
const BudgetComparisonChart = ({ analyticsData = [] }) => {
  ...
  if (!analyticsData || analyticsData.length === 0) {
    return (
      <div className="p-8 text-center text-xs text-slate-400 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/70">
        No category budgets configured for this month to display comparison chart.
      </div>
    );
  }
    ...
    <div className="p-6 rounded-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/70 shadow-soft space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Visual comparison of category budget targets against real expenses
        </p>
      </div>
      ...
      <div className="h-72 w-full pt-4">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Bar dataKey="Budget" name="Budget Limit" fill="#6366f1" radius={[4, 4, 0, 0]} maxBarSize={28} />
            <Bar dataKey="Actual" name="Actual Spent" fill="#f43f5e" radius={[4, 4, 0, 0]} maxBarSize={28} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
