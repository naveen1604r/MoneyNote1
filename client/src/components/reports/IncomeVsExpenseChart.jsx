...
const IncomeVsExpenseChart = ({ monthlyData = [] }) => {
  if (!monthlyData || monthlyData.length === 0) {
    return (
      <div className="p-8 text-center text-xs text-slate-400">
        No monthly income vs expense records found for the selected date range.
      </div>
    );
  }
    ...
    <div className="p-6 rounded-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/70 shadow-soft space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Monthly breakdown comparing total earnings vs total spending
        </p>
      </div>
